...
CSV의 추가 FAR 필드 전송은 [CSV 가져오기 안내](csv-import.md)에 정리했습니다. 기본 입력 폴더는 프로젝트의 `csv/`이며 `CSV_DIR`로 변경할 수 있습니다. 등록한 CSV 작업은 웹과 Ubuntu worker가 기존 큐에서 처리합니다.
...
코드를 업데이트할 때는 서비스를 멈추고 백업한 다음 설치합니다. 실행 중인 POST/PATCH는 중단 시 `확인 대기`가 될 수 있으므로 가능하면 활성 작업이 끝난 뒤 중지하세요.

로컬 전송 이력에 사용할 업무 키가 다르면 `TARGET_KEY_FIELDS`를 먼저 맞춥니다. 서버 PATCH의 `where`는 이 설정과 관계없이 `far_no`와 `sample_no` 두 칸으로 고정됩니다. 설정 변경으로 기존 작업 대상이 달라지면 이전 대기 작업은 실패 처리되며 다른 대상으로 전송되지 않습니다. 스케줄을 다시 저장하세요.
...
우측 요청·응답 패널에서 POST 400과 후속 PATCH의 실제 응답을 각각 확인할 수 있습니다. PATCH가 성공하면 실행은 성공으로 완료되고 다음 실행의 동일한 값은 건너뜁니다. POST와 PATCH는 각각 다른 Idempotency-Key를 사용합니다. 네트워크 오류·5xx·202·확인 응답 오류 등 반영 여부가 불확실하면 자동 반복하지 않고 전송 확인 대기로 남깁니다.

### 전송 확인 대기 처리

1. **전송 확인 대기**에서 원래 요청·응답을 열고 대상 서버의 업무 키와 전송한 모든 값을 대조합니다. 원래 실패가 PATCH였다면 해당 PATCH의 `where`·`values`와 응답을 표시합니다.
2. 확인한 항목을 목록에서 체크합니다. 같은 결과인 여러 항목을 함께 선택할 수 있습니다.
3. **선택 항목 반영 완료** 또는 **선택 항목 미반영**을 누릅니다. 사유 입력 없이 선택한 결과를 저장합니다.

반영됨은 성공 상태로 기록해 같은 값을 다시 보내지 않습니다. 미반영은 다음 수동 실행 또는 예약 실행에서 다시 전송할 수 있게 합니다. 처리 버튼을 누르는 순간 서버로 재전송하거나 실행을 새로 등록하지는 않습니다. 실행 중인 항목이나 선택 후 상태가 바뀐 항목이 포함되어 있으면 모두 유지하고 다시 확인하도록 표시합니다. 이전 실행의 HTTP 기록과 오류는 보존합니다.

## 네트워크·인증

- 각 API의 인증은 `CLAIMS_HEADERS`, `PRODUCT_HEADERS`, `TARGET_HEADERS`에 JSON 객체로 설정합니다. 인증 값은 화면이나 오류 로그에서 가립니다. 요청·응답 상세에서는 Content-Type 같은 일반 헤더를 확인할 수 있습니다.
...
인증서를 설정해도 기존 **전송 확인 대기** 항목은 자동 재전송하지 않습니다. 원래 요청·응답과 서버 반영 여부를 확인하고 웹의 전송 확인 대기 목록에서 항목을 체크해 반영됨·미반영으로 처리한 뒤 재실행하세요.
...
웹 오른쪽 **요청·응답 로그** 패널에서 성공·실패 요청을 모두 확인할 수 있습니다. 넓은 화면에서는 처음부터 열리며, 좁은 화면에서는 상단 버튼으로 엽니다. 실행 현황의 **요청·응답 상세**, 레코드 상세의 **요청 이력 보기**, 전송 확인 대기의 **원래 요청·응답**에서도 바로 열 수 있습니다.

- 선택한 실행, 전체 실행, API 연결 확인 범위를 고르고 `실패·보류만`으로 필터링할 수 있습니다. 목록은 최신순으로 페이지를 나눠 표시합니다.
...
공유 처리 엔진이 요청하기 전에 SQLite의 `http_exchanges`에 기록하고, 응답을 받으면 결과를 갱신합니다. 웹·Ubuntu worker 모두 동일하게 저장합니다. 시작 시 필요한 테이블·인덱스와 CSV 행 번호·업무 키·안내 컬럼을 추가하며 기존 실행 이력과 전송 기록은 유지합니다. CSV 중복 행은 각 행의 결과를 따로 보관하며 상세 HTTP 목록에도 CSV 행 번호를 표시합니다.
...
- 실패 단계, GET/POST/PATCH 방식, **실제로 요청한 전체 URL**. Claim 조회의 `rcvDateFrom`, `rcvDateTo`, `limit`와 제품 조회의 인코딩된 part ID 경로를 포함합니다.
- HTTP 상태 코드와 상태 설명, 응답 Content-Type.
- 서버가 제공한 `x-request-id`, `x-correlation-id`, `traceparent`. API 담당자가 서버 로그와 대조할 때 사용할 수 있습니다.
- 서버 응답 본문. JSON은 읽기 쉽게 표시하고 HTML·일반 텍스트 오류도 보존합니다. 간단한 오류 메시지는 마스킹 후 최대 4,096자까지 표시하며 초과분은 `[응답 일부 생략]`으로 표시합니다. 우측 상세 패널은 `HTTP_LOG_BODY_BYTES` 한도까지 보관합니다.
- 연결 실패·시간 초과의 예외 종류와 원인, GET 재시도 횟수. POST/PATCH의 자동 재전송 보류 여부도 표시합니다.
...
간단한 오류 메시지에는 요청 헤더와 전송 데이터를 반복해서 넣지 않고 우측 패널에서 상세를 제공합니다. 설정한 인증 헤더 값이 응답에 그대로 되돌아오면 가리며, 토큰·비밀번호·쿠키·API key 항목과 URL 인증 정보도 `[REDACTED]`로 표시합니다. 일반 업무 필드와 서버 메시지는 원인 파악을 위해 남깁니다.

작업 오류는 `DATA_DIR/claim-sync.sqlite3`의 `events`, `chunks.error`, `records.error` 등에 저장되므로 웹을 종료해도 보존됩니다. `http_exchanges`도 별도 자동 삭제 없이 누적되며 응답에 업무 데이터가 포함되므로 DB 접근 권한과 보관 정책을 같이 관리하세요.
