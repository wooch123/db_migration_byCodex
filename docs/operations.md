...
- 요청을 선택하면 요청 URL, GET 쿼리, POST JSON, 인증정보를 가린 요청·응답 헤더, 실제 응답 코드·본문, 요청 시각과 소요 시간, 재시도 순서를 확인합니다. `목록 접기`로 상세 영역을 넓힐 수 있습니다.
- `최신 요청 따라가기`를 끄거나 과거 요청을 선택하면 상세 내용을 읽는 동안 선택이 유지됩니다. `전체 복사`와 `JSON 저장`은 선택한 요청의 상세 기록을 내보냅니다.
- 연결·SSL 오류로 HTTP 응답을 받지 못하면 응답 코드가 없음을 표시합니다. 이전 전송 때문에 이번 POST를 보류했으면 `전송 안 함`으로 표시하고 원래 실패한 실행의 오류, POST 데이터와 상세 기록을 연결합니다. 기록을 열어보는 동작은 재전송하지 않습니다.
...
요청·응답 본문 저장 한도는 `.env`의 `HTTP_LOG_BODY_BYTES`이며 기본 262,144 bytes(256 KiB), 최대 10 MiB입니다. 한도보다 큰 본문은 앞부분과 생략 여부를 표시합니다. 더 많이 보관해야 한다면 값을 늘리고 재시작하세요. 설정 변경은 이후 요청부터 적용됩니다. 이 한도는 로그 저장용이며 실제 전송 JSON을 자르거나 `MAX_RESPONSE_BYTES`의 조회 처리 한도를 변경하지 않습니다.
